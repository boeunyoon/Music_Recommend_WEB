...
const TrackModalSearch = ({show, playlist, onClose}) => {
  ...
  return (
    <Modal show={show} onHide={onClose} size="lg" className="modal-container">
      ...
      <Row className = "modal-title">
          <Col><Image src={playlist.image640} rounded className = "playlist-image"/></Col>
          <Col>
            <Row>
              <h2>{playlist.title}</h2>
              <h4>{playlist.artist}</h4>
              <h4>Album: {playlist.album}</h4>
              <h4>release date: {playlist.releaseDate}</h4>
            </Row>
          </Col>
        </Row>
      </Modal.Body>
...
export default TrackModalSearch